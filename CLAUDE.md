...
- **Confirmation popups**: every call-to-action confirmation (complete, save, discard, delete…) uses `.confirmationAlert(_:isPresented:message:confirmLabel:confirmRole:onConfirm:)` from `Core/UI/ConfirmationAlert.swift` — a centered system alert with a title, a message, and Cancel + the action side by side. Never use `.confirmationDialog`: it anchors to its trigger (or the top of the page) instead of centering. Raw `.alert` is reserved for error alerts (Retry/Dismiss/OK) and prompts with three or more choices.
- **Program runs**: a `UserProgram` row is one *run* through a program, not the program itself. `completedAt` / `archivedAt` are terminal. "Start again" is just `activateProgram` on the completed run — the server opens a fresh run at week 1 day 1 and returns a **different id**, so always refetch after activating rather than reusing the id you sent. Treat a run as active only when `isActive && completedAt == nil` (`isActiveRun`). "End program early" is `completeProgram` (`PATCH /api/user-programs/:id/complete`) on an open run, active or paused: terminal (unlike deactivate, which pauses), it deletes the run's unfinished sessions and scheduled workouts and keeps completed ones. It 409s on a terminal run and on a run with no completed workouts, so the action stays hidden until one exists; restarting afterwards is a separate `activateProgram`. Any mutation that changes the current run calls `ProgramRunChanges.notify()` so Home and the resume banner refetch instead of keeping state loaded under the old run id. Server contract: `docs/API_CONTRACT_RUNS_AND_EDITING.md` in the server repo.
- **Editing workouts**: any owned workout session is editable in any status, with or without an active program (server contract: `docs/API_CONTRACT_EDITING_COMPLETED_WORKOUTS.md`). `ProgramDayEditView` is the shared edit screen, reached from both Manage Program and a completed workout's detail view (History / Home). The two server-side exceptions: `/complete` and `DELETE /api/workouts/:id` both 409 on a `COMPLETED` session, so the edit view hides Discard for completed sessions and saves date changes via `PATCH /api/workouts/:id`.
...
- **Core workout delete UI.** `deleteCoreWorkout` is wired in `WorkoutRepository`, but `CoreView` has no UI trigger for it — no way to remove a saved circuit once saved (only Edit to unlock and re-save). Add a "Remove core workout" action alongside `saveEditButton`/`startButton` that calls the delete endpoint and clears local Core state.
- **Report handled `APIError`s to Sentry.** Handled Swift errors are never auto-captured — Sentry only auto-reports crashes, unhandled exceptions, hangs, and failed (error-status) HTTP requests, and OSLog doesn't bridge to Sentry. So errors like the feedback POST decode failure (201 response, `DecodingError` caught in `APIClient`, surfaced only as a UI string) are invisible in Sentry. Add `SentrySDK.capture(error:)` in `APIClient`'s decode catch blocks (tag with the endpoint path); `.decoding` errors are the priority since they always mean a client/server contract violation.
...
- **Email auth deep links.** Email sign-in shipped without universal links: the Supabase confirmation/reset emails open the *web* pages, and iOS shows "check your email, then sign in" (`AuthView` form modes + `pendingConfirmationEmail` resend affordance). Deep-linking them into the app needs an associated-domains entitlement, an AASA file served by the Nuxt server, a bounce page (Supabase puts tokens in a URL hash fragment, which universal links don't deliver), an in-app reset screen, and Supabase redirect-allowlist entries.
