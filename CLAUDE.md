...
- **Email sign-in provider.** Web supports email alongside Apple and Google; iOS has Apple and Google (`SignInWithAppleButton` / `SignInWithGoogleButton`, both wired through `AuthRepository`). Email auth still needs sign-in / sign-up / password-reset forms, the matching `APIEndpoint` cases and `AuthRepository` methods, plus deep-link handling for the email-confirmation redirect.
